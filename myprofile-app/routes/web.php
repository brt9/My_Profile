...
Route::middleware(['auth', 'verified', 'portfolio.admin'])->prefix('admin/calendar')->name('calendar.')->group(function () {
    ...
    Route::post('/sync', [GoogleCalendarOAuthController::class, 'sync'])->name('sync');
    Route::delete('/revoke', [GoogleCalendarOAuthController::class, 'revoke'])->name('revoke');
});

Route::middleware(['auth', 'verified', 'portfolio.admin'])->prefix('api/calendar/events')->name('calendar.events.')->group(function () {
